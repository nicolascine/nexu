...
// Usage:
//   pnpm context -- --path /path/to/project
//   pnpm context -- --path /path/to/project --copy
//   pnpm context -- --path /path/to/project --format json
...
function getLastCommit(path: string): SessionContext['git']['lastCommit'] {
...
function getRecentCommits(path: string, count: number = 5): SessionContext['git']['recentCommits'] {
  const log = exec(`git log -${count} --format="%H|%s|%ar"`, path);
  if (!log) return [];
...
function getUncommittedChanges(path: string): string[] {
...
function getStashes(path: string): string[] {
  const stashes = exec('git stash list', path);
  if (!stashes) return [];
  return stashes.split('\n').filter(Boolean);
}

function findTodos(path: string): SessionContext['pendingItems'] {
  const todos: SessionContext['pendingItems'] = [];
  const extensions = ['.ts', '.tsx', '.js', '.jsx', '.py', '.go', '.rs'];
  const ignorePatterns = ['node_modules', '.git', 'dist', 'build', '.next', '__pycache__', 'venv', '.nexu'];
...
function getRecentFiles(path: string): SessionContext['recentFiles'] {
  // Get files modified in the last 24 hours
  const oneDayAgo = Date.now() - 24 * 60 * 60 * 1000;
  const recentFiles: SessionContext['recentFiles'] = [];
  const extensions = ['.ts', '.tsx', '.js', '.jsx', '.py', '.go', '.rs', '.json', '.md'];
  const ignorePatterns = ['node_modules', '.git', 'dist', 'build', '.next', 'package-lock', 'pnpm-lock'];

  // Use git to find recently modified files
  const gitFiles = exec('git diff --name-status HEAD~10 HEAD 2>/dev/null || git diff --name-status --cached', path);
  if (gitFiles) {
  ...
  return recentFiles.slice(0, 15);
}

function inferCurrentTask(context: Partial<SessionContext>): { summary: string; branch: string } {
...
function inferNextStep(context: SessionContext): string {
  // Priority: uncommitted changes > TODOs > stashes > generic
...
function generateMarkdown(context: SessionContext): string {
  const lines: string[] = [];
  ...
  if (context.git.lastCommit) {
    ...
    lines.push(`- **Last commit:** ${context.git.lastCommit.message} (\`${context.git.lastCommit.hash}\`, ${context.git.lastCommit.date})`);
...
async function main() {
  ...
  const copyToClipboard = hasFlag('copy');

  if (!targetPath) {
    console.log('');
    console.log('╔═══════════════════════════════════════╗');
    console.log('║       nexu session context            ║');
    console.log('╚═══════════════════════════════════════╝');
    console.log('');
    console.log('Usage:');
    console.log('  pnpm context -- --path /path/to/project');
    console.log('  pnpm context -- --path . --copy');
    console.log('  pnpm context -- --path ~/src/myapp --format json');
    console.log('');
    console.log('Options:');
    console.log('  --path <path>     Project directory');
    console.log('  --format <fmt>    Output format: markdown (default), json');
  ...
  if (copyToClipboard) {
    try {
      execSync('pbcopy', { input: output, encoding: 'utf-8' });
      console.log('✓ Context copied to clipboard!');
      console.log('');
      console.log('Paste it into Claude Code, Cursor, or any AI assistant.');
    } catch {
